...
from ..remainderapp.models import Appt
...
def success(request):
	...
	return render(request,'remainderapp/success.html',context)
def user(request):
	errors = False
	...
	if errors == True:
		...
		Userlog.UserManager.create(name=check1[1], username=check2[1], password=check3[1],birthday=check5[1])
		request.session['user'] = check1[1]
		print
		return redirect('/appointments')

def login(request):
	check6 = Userlog.UserManager.log(request.POST['username'], request.POST['password'])
	if check6[0] == False:
