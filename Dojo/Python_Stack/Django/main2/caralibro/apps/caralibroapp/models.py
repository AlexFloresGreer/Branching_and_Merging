...
class validationManager(models.Manager):
    ...
    def validateName(self, request, first_name, alias):
        no_error = True
        if len(first_name) < 2 or any(char.isdigit() for char in first_name):
            ...
            no_error = False
        if len(alias) < 2 or any(char.isdigit() for char in alias):
            messages.error(request, 'Alias must be 2 characters and only letters')
            no_error = False
        return no_error
...
class Poke(models.Model):
    pokee = models.IntegerField()
    poker = models.ForeignKey(User)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    objects = models.Manager()
